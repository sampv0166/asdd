...
const AddProductScreen = ({ history, match }) => {
  ...
  const addToVariationList = (formik) => {
    if (ProductVariationList.length >= 1) {
      if (
        (ProductVariationList[0].color_name !== null ||
          ProductVariationList[0].color_name !== "") &&
        (formik.values.color_name === null || formik.values.color_name === "")
      ) {
        alert("this product should have a color");
        return;
      }

      if (
        (ProductVariationList[0].size_value !== null ||
          ProductVariationList[0].size_value !== "") &&
        (formik.values.size_value === null || formik.values.size_value === "")
      ) {
        alert("this product should have a size");
        return;
      }
      ...
      if (
        ...
        alert("this product do not have a color variant");
        return;
      }
      ...
      if (
        ...
        alert("this product do not have a size variant");
        return;
      }
    }
  ...
  const handleformdata = (values, resetForm) => {
    ...
    handleSubmit(formdata, values, arr);
  };

  const handleSubmit = async (formdata, values, arr) => {
      await dispatch(
        createProduct(
          dispatch,
          formdata,
